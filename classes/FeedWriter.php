...
class FeedWriter {
  ...
  /**
   * Set the 'selfLink' channel element
   * 
   * @access   public
   * @param    string  value of 'selfLink' channel tag
   * @return   void
   */
  public function setSelfLink($link) {
    $this->setChannelElement('selfLink', $link);
  } 
  ...
  private function makeNode($tagName, $tagContent, $attributes = null) {
    ...
    $attrText .= (in_array($tagName, $this->CDATAEncoding) && $this->version == ATOM || !is_array($tagContent) && $tagContent != utf8_to_html($tagContent)) ? ' type="html" ' : '';
    ...
    if (is_array($tagContent)) {
      ...
      $nodeText .= (in_array($tagName, $this->CDATAEncoding)) ? $tagContent : utf8_to_html($tagContent);
    }
  ...
  private function printChannels() {
    ...
    foreach ($this->channels as $key => $value) {
      ...
      elseif ($this->version == ATOM && $key == 'selfLink') {
        echo $this->makeNode('link', '', array('href' => utf8_to_html($value), 'rel' => 'self', 'type' => 'application/atom+xml'));
      }
      else {
